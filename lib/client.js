...
export async function createCustomObject(container, key, values) {
...
export async function getCustomObjectsByContainer(conainer = null) {
  const accessToken = await getToken();
  const collectionUrl = `${apiUrl}/${projectKey}/custom-objects${conainer}`;
  const res = await fetch(collectionUrl, {
    method: 'get',
    headers: {
      'Authorization': `Bearer ${accessToken}`
...
/* 
  NOTE: Funcs below are optional, in conjunction with an optional API. Possible future state use
*/
